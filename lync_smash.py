			...
			if timeout:
				...
				with open(os.path.abspath(args.usernames)) as user_file:
					for user in user_file:
						response_time = send_xml(args.host, args.domain, user.rstrip())
	...
	# Lock user's Lync account
		...
		for lock in range(1, 6):
			try:
				...
				response_time = send_xml(args.host, args.domain, args.user)
...
def baseline_timeout(host, domain):
	...
	for loop in range(0, 3):
		try:
			...
			response_time = send_xml(host, domain, random_user)
			if response_time:
...
def send_xml(host, domain, user):
	...
	try:
		...
		response_time = requests.post(lync_url, headers=headers, data=xml_data, verify=False).elapsed.total_seconds()
	except Exception as error:
		return None
